...
systemTest = forcefield.createSystem(pdb.topology, nonbondedMethod=app.CutoffPeriodic, 
    nonbondedCutoff=1.0*nanometers, constraints=app.HBonds, rigidWater=True, 
    ewaldErrorTolerance=0.0005)

print "this is a template TIP3P CutoffPeriodic energy test for ", InFile
...
print "Total CutoffPeriodic Energy: ", simulationTest.context.getState(getEnergy=True, groups=2).getPotentialEnergy()
print "Total Forces: " 
...
simulationTest = app.Simulation(pdb.topology, systemTest, integratorTest, platform)
simulationTest.context.setPositions(pdb.positions)
print "Direct Space CutoffPeriodic Energy: ", simulationTest.context.getState(getEnergy=True, groups=2).getPotentialEnergy()
print "Direct Space Forces: " 
forces = simulationTest.context.getState(getEnergy=True, getForces=True, groups=2).getForces()
for i in range(len(forces)):
    print "particle ", i, forces[i] 
integratorTest = mm.VerletIntegrator(0.002)
